...
import time
import torch
from Pytorch.Chemistry.data import NumpyDataset
...
class ClippingTransformer(Transformer):
  """Clip large values in datasets.
     ...
     >>> dataset = Pytorch.Chemistry.data.NumpyDataset(X, y, w, ids)
     >>> transformer = Pytorch.Chemistry.trans.ClippingTransformer(transform_X=True)
...
class CoulombFitTransformer(Transformer):
  """Performs randomization and binarization operations on batches of Coulomb Matrix features during fit.
     ...
     >>> model = Pytorch.Chemistry.models.MultitaskFitTransformRegressor(n_tasks,
  ...
  def __init__(self, dataset):
    ...
    Parameters:
    ----------
    dataset: Pytorch.Chemistry.data.Dataset object
    """
    X = dataset.X
    num_atoms = X.shape[1]
    self.step = 1.0
...
class ANITransformer(Transformer):
  ...
  def distance_matrix(self, coordinates, flags):
    ...
    max_atoms = self.max_atoms
    tensor1 = torch.stack([coordinates] * max_atoms, axis=1)
    tensor2 = torch.stack([coordinates] * max_atoms, axis=2)

    # Calculate pairwise distance
    d = torch.sqrt(torch.sum(torch.mul(tensor1 - tensor2), axis=3))
  ...
  def distance_cutoff(self, d, cutoff, flags):
    ...
    d_flag = torch.nn.ReLU(d_flag)
    d_flag = d_flag * torch.nn.utils.addSingletonDimension((1 - torch.eye(self.max_atoms)), 0)
    d = 0.5 * (torch.cos(np.pi * d / cutoff) + 1)
    return d * d_flag

  def radial_symmetry(self, d_cutoff, d, atom_numbers):
    """ Radial Symmetry Function """
    embedding = torch.eye(torch.max(self.atom_cases) + 1)
    atom_numbers_embedded = np.take(embedding, atom_numbers)
    ...
    d_cutoff = torch.stack([d_cutoff] * length, axis=3)
    d = torch.stack([d] * length, axis=3)

    out = torch.exp(-ita * torch.square(d - Rs)) * d_cutoff
    if self.atomic_number_differentiated:
      out_tensors = []
      for atom_type in self.atom_cases:
        selected_atoms = torch.nn.utils.addSingletonDimension(
            torch.nn.utils.addSingletonDimension(atom_numbers_embedded[:, :, atom_type], axis=1),
            axis=3)
        out_tensors.append(torch.sum(out * selected_atoms, axis=2))
      return torch.cat(out_tensors, axis=2)
    else:
      return torch.sum(out, axis=2)
  ...
  def angular_symmetry(self, d_cutoff, d, atom_numbers, coordinates):
    ...
    R_ij = torch.stack([d] * max_atoms, axis=3)
    R_ik = torch.stack([d] * max_atoms, axis=2)
    f_R_ij = torch.stack([d_cutoff] * max_atoms, axis=3)
    f_R_ik = torch.stack([d_cutoff] * max_atoms, axis=2)

    # Define angle theta = arccos(R_ij(Vector) dot R_ik(Vector)/R_ij(distance)/R_ik(distance))
    vector_mul = torch.sum(torch.stack([vector_distances] * max_atoms, axis=3) * \
                               torch.stack([vector_distances] * max_atoms, axis=2), axis=4)
    vector_mul = vector_mul * torch.sign(f_R_ij) * torch.sign(f_R_ik)
    theta = torch.acos(torch.div(vector_mul, R_ij * R_ik + 1e-5))

    R_ij = torch.stack([R_ij] * length, axis=4)
    R_ik = torch.stack([R_ik] * length, axis=4)
    f_R_ij = torch.stack([f_R_ij] * length, axis=4)
    f_R_ik = torch.stack([f_R_ik] * length, axis=4)
    theta = torch.stack([theta] * length, axis=4)

    out_tensor = torch.pow((1. + torch.cos(theta - thetas)) / 2., zeta) * \
                 torch.exp(-ita * torch.square((R_ij + R_ik) / 2. - Rs)) * f_R_ij * f_R_ik * 2

    if self.atomic_number_differentiated:
      out_tensors = []
      for id_j, atom_type_j in enumerate(self.atom_cases):
        for atom_type_k in self.atom_cases[id_j:]:
          selected_atoms = torch.stack([atom_numbers_embedded[:, :, atom_type_j]] * max_atoms, axis=2) * \
                           torch.stack([atom_numbers_embedded[:, :, atom_type_k]] * max_atoms, axis=1)
          selected_atoms = torch.nn.utils.addSingletonDimension(
              torch.nn.addSingletonDimension(selected_atoms, axis=1), axis=4)
          out_tensors.append(
              torch.sum(out_tensor * selected_atoms, axis=(2, 3)))
      return torch.cat(out_tensors, axis=2)
    else:
      return torch.sum(out_tensor, axis=(2, 3))
