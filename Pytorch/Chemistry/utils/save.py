...
Simple utils to save and load from disk for Pytorch
    ...
    if shard_size is None:
      ...
      for df in pd.read_csv(filename, chunksize=shard_size):
        log("Loading shard %d of size %s." % (shard_num, str(shard_size)),
            verbose)
        df = df.replace(torch.Tensor(float('nan')), str(""), regex=True) # np.nan was used but replaced by torch
        shard_num += 1
        yield df
...
def seq_one_hot_encode(sequences, letters='ATCGN'):
  ...
  return np.expand_dims(torch.Tensor(seqs), -1)
...
def encode_fasta_sequence(fname):
  ...
  Returns
  -------
  torch.Tensor: Shape (N_sequences, 5, sequence_length, 1).
  """
...
def encode_bio_sequence(fname, file_type="fasta", letters="ATCGN"):
  ...
  Returns
  -------
  torch.Tensor: Shape (N_sequences, N_letters, sequence_length, 1).
  """
...
def load_dataset_from_disk(save_dir):
  ...
  loaded = True
  train = Pytorch.Chemistry.DiskDataset(train_dir)
  valid = Pytorch.Chemistry.DiskDataset(valid_dir)
  test = Pytorch.Chemistry.DiskDataset(test_dir)
