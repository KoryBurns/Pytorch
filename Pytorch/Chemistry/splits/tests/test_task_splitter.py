...
class TestTaskSplitters(unittest.TestCase):
  ...
  def test_multitask_train_valid_test_split(self):
    ...
    dataset = pc.data.NumpyDataset(X, y)

    task_splitter = pc.splits.TaskSplitter()
  ...
  def test_merge_fold_datasets(self):
    ...
    dataset = pc.data.NumpyDataset(X, y, w)
    K = 5
    ...
    for fold in range(K):
      ...
      train_dataset = pc.splits.merge_fold_datasets(train_fold_datasets)
