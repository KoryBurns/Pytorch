...
from Pytorch.Chemistry.utils import pad_array
from Pytorch.Chemistry.features.atomic_coordinates import AtomicCoordinates
...
class CoulombMatrix(Featurizer):
  ...
  seed : int, optional
      Random seed.
  Example:
  >>> featurizers = Pytorch.Chemistry.features.CoulombMatrix(max_atoms=23)
  >>> input_file = 'Pytorch/Chemistry/features/tests/data/water.sdf' # really backed by water.sdf.csv
  >>> tasks = ["atomization_energy"]
  >>> featurizer = Pytorch.Chemistry.data.SDFLoader(tasks, smiles_field="smiles", mol_field="mol",
  ...                                    featurizer=featurizers, verbose=False)
  >>> dataset = featurizer.featurize(input_file)
  Reading structures from Pytorch/Chemistry/features/tests/data/water.sdf.
...
class CoulombMatrixEig(CoulombMatrix):
  ...
  seed : int, optional
      Random seed.
  Example:
  >>> featurizers = Pytorch.Chemistry.features.CoulombMatrixEig(max_atoms=23)
  >>> input_file = 'Pytorch/Chemistry/features/tests/data/water.sdf' # really backed by water.sdf.csv
  >>> tasks = ["atomization_energy"]
  >>> featurizer = Pytorch.Chemistry.data.SDFLoader(tasks, smiles_field="smiles", mol_field="mol",
  ...                                    featurizer=featurizers, verbose=False)
