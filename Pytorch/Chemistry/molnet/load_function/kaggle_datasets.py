...
from Pytorch.Chemistry.molnet.load_function.kaggle_features import merck_descriptors
...
def get_transformers(train_dataset):
  ...
  transformers = []
  #transformers = [
  #    pc.trans.LogTransformer(transform_X=True),
  #    pc.trans.NormalizationTransformer(transform_y=True,
...
def gen_kaggle(KAGGLE_tasks,
  ...
  featurizer = pc.features.UserDefinedFeaturizer(merck_descriptors)

  loader = pc.data.UserCSVLoader(
...
def load_kaggle(shard_size=2000, featurizer=None, split=None, reload=True):
  ...
  if (os.path.exists(train_dir) and os.path.exists(valid_dir) and
    ...
    train_dataset = pc.data.DiskDataset(train_dir)
    valid_dataset = pc.data.DiskDataset(valid_dir)
    test_dataset = pc.data.DiskDataset(test_dir)
  else:
