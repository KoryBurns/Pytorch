...
import pickle
from Pytorch.Chemistry.molnet.run_benchmark_models import benchmark_classification, benchmark_regression
from Pytorch.Chemistry.molnet.check_availability import CheckFeaturizer, CheckSplit
...
def run_benchmark(datasets,
  ...
  for dataset in datasets:
    if dataset in [
      ...
      if metric == None:
        metric = [
            pc.metrics.Metric(pc.metrics.roc_auc_score, np.mean),
        ]
    elif dataset in [
      ...
      if metric == None:
        metric = [
            pc.metrics.Metric(pc.metrics.pearson_r2_score, np.mean)
        ]
    else:
    ...
    loading_functions = {
        'bace_c': pc.molnet.load_bace_classification,
        'bace_r': pc.molnet.load_bace_regression,
        'bbbp': pc.molnet.load_bbbp,
        'chembl': pc.molnet.load_chembl,
        'clearance': pc.molnet.load_clearance,
        'clintox': pc.molnet.load_clintox,
        'delaney': pc.molnet.load_delaney,
        'hiv': pc.molnet.load_hiv,
        'hopv': pc.molnet.load_hopv,
        'kaggle': pc.molnet.load_kaggle,
        'lipo': pc.molnet.load_lipo,
        'muv': pc.molnet.load_muv,
        'nci': pc.molnet.load_nci,
        'pcba': pc.molnet.load_pcba,
        'pcba_146': pc.molnet.load_pcba_146,
        'pcba_2475': pc.molnet.load_pcba_2475,
        'pdbbind': pc.molnet.load_pdbbind_grid,
        'ppb': pc.molnet.load_ppb,
        'qm7': pc.molnet.load_qm7_from_mat,
        'qm7b': pc.molnet.load_qm7b_from_mat,
        'qm8': pc.molnet.load_qm8,
        'qm9': pc.molnet.load_qm9,
        'sampl': pc.molnet.load_sampl,
        'sider': pc.molnet.load_sider,
        'tox21': pc.molnet.load_tox21,
        'toxcast': pc.molnet.load_toxcast
    }
    ...
    if hyper_param_search:
      ...
      search_mode = pc.hyper.GaussianProcessHyperparamOpt(model)
...
def load_dataset(dataset, featurizer, split='random'):
  ...
  dataset_loading_functions = {
      ...
      'nci': pc.molnet.load_nci,
      'pcba': pc.molnet.load_pcba,
      'pcba_128': pc.molnet.load_pcba_128,
      'pcba_146': pc.molnet.load_pcba_146,
      'pcba_2475': pc.molnet.load_pcba_2475,
      'pdbbind': pc.molnet.load_pdbbind_grid,
      'ppb': pc.molnet.load_ppb,
