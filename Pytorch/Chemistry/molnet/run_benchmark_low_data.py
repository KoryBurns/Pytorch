...
import torch
from Pytorch.Chemistry.molnet.run_benchmark_models import low_data_benchmark_classification
from Pytorch.Chemistry.molnet.check_availability import CheckFeaturizer
...
def run_benchmark_low_data(datasets,
  ...
  for dataset in datasets:
    ...
    metric_all = {
        'auc': pc.metrics.Metric(pc.metrics.roc_auc_score, np.mean)
    }
    ...
    for count_iter in range(num_iter):
      ...
      train_folds = all_dataset[:K - count_iter - 1] + all_dataset[K -
                                                                   count_iter:]
      train_dataset = pc.splits.merge_fold_datasets(train_folds)
