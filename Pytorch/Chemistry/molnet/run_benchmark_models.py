...
from Pytorch.Chemistry.molnet.preset_hyper_parameters import hps
...
def benchmark_classification(train_dataset,
  ...
  assert model in [
      'rf', 'torch', 'torch_robust', 'logreg', 'irv', 'graphconv', 'dag', 'xgb',
      'weave', 'kernelsvm', 'textcnn', 'mpnn'
  ]
  ...
  if model_name == 'torch':
    ...
    model = pc.models.MultitaskClassifier(
        len(tasks),
        n_features,
    ...
    model = pc.models.RobustMultitaskClassifier(
        len(tasks),
        n_features,
        layer_sizes=layer_sizes,
        weight_init_stddevs=weight_init_stddevs,
    ...
    def model_builder(model_dir_logreg):
      sklearn_model = LogisticRegression(
          ...
          n_jobs=-1)
      return pc.models.sklearn_models.SklearnModel(
          sklearn_model, model_dir_logreg)

    model = pc.models.multitask.SingletaskToMultitask(
        tasks, model_builder)
    ...
    transformer = pc.trans.IRVTransformer(n_K, len(tasks), train_dataset)
    ...
    model = pc.models.TorchMultitaskIRVClassifier(
        len(tasks),
        K=n_K,
        penalty=penalty,
        batch_size=batch_size,
    ...
    model = pc.models.GraphConvModel(
        len(tasks),
    ...
    reshard_size = 256
    transformer = pc.trans.DAGTransformer(max_atoms=max_atoms)
    ...
    all_data = pc.data.DiskDataset.merge(
        [train_dataset, valid_dataset, test_dataset])
    char_dict, length = pc.models.TextCNNModel.build_char_dict(all_data)

    model = pc.models.TextCNNModel(
        len(tasks),
        char_dict,
        seq_length=length,
    ...
    def model_builder(model_dir_kernelsvm):
      ...
      return pc.models.SklearnModel(sklearn_model, model_dir_kernelsvm)
    ...
    def model_builder(model_dir_xgb):
      ...
      xgboost_model = xgboost.XGBClassifier(
          ...
          seed=seed)
      return pc.models.xgboost_models.XGBoostModel(
...
def benchmark_regression(train_dataset,
  ...
  assert model in [
      'torch_regression', 'torch_regression_ft', 'rf_regression', 'graphconvreg',
      'dtnn', 'dag_regression', 'xgb_regression', 'weave_regression',
      'textcnn_regression', 'krr', 'ani', 'krr_ft', 'mpnn'
  ]
  import xgboost
  ...
  if model_name == 'torch_regression':
    layer_sizes = hyper_parameters['layer_sizes']
    weight_init_stddevs = hyper_parameters['weight_init_stddevs']
    bias_init_consts = hyper_parameters['bias_init_consts']
    dropouts = hyper_parameters['dropouts']
    ...
    model = pc.models.MultitaskRegressor(
        len(tasks),
        n_features,
        layer_sizes=layer_sizes,
        weight_init_stddevs=weight_init_stddevs,
  ...
  elif model_name == 'torch_regression_ft':
    layer_sizes = hyper_parameters['layer_sizes']
    weight_init_stddevs = hyper_parameters['weight_init_stddevs']
    bias_init_consts = hyper_parameters['bias_init_consts']
    dropouts = hyper_parameters['dropouts']
    ...
    model = pc.models.MultitaskFitTransformRegressor(
        n_tasks=len(tasks),
    ...
    char_dict, length = pc.models.TextCNNModel.build_char_dict(
        train_dataset)
    ...
    try:
      ...
      pass
    ANItransformer = pc.trans.ANITransformer(
    ...
    def model_builder(model_dir_krr):
      ...
      return pc.models.SklearnModel(sklearn_model, model_dir_krr)
    ...
    ft_transformer = pc.trans.CoulombFitTransformer(train_dataset)
